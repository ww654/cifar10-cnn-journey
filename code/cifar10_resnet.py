...
log_path = os.path.join(log_dir, f'resnet_run_{timestamp}.txt')
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        # ★ 核心：主路径的输出 + 原始输入（捷径）
        return self.relu(self.path(x) + x)
...
class ResNet_Small(nn.Module):
    def __init__(self):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(64, 128, 3, stride=2, padding=1, bias=False),  # 下采样
            nn.BatchNorm2d(128),
            nn.ReLU(),
            ResidualBlock(128),
            ResidualBlock(128),
        ...
        self.layer3 = nn.Sequential(
            nn.Conv2d(128, 256, 3, stride=2, padding=1, bias=False),  # 下采样
            nn.BatchNorm2d(256),
            nn.ReLU(),
            ResidualBlock(256),
            ResidualBlock(256),
        ...
        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),   # 把任意大小的特征图压成 1x1
            nn.Flatten(),
            nn.Linear(256, 10),
        )
